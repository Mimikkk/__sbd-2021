...
import { DateSelect, Form, SelectField, TextField, TimePicker } from 'shared/components';
...
export const ItemReservationForm = <T extends ItemReservation.Model>({
  ...
  const handleRemove = async (values: T) => (
    await (isEntity(values) && itemReservationService.delete(values.id)), refresh()
  );

  const {date, setDate} = useFormReducer();

  const [items, setItems] = useState<Item.Entity[]>([]);
  useEffect(() => {itemService.readAll().then(({ items }) =>
    setItems(items.filter(({ name, id}) => name))
  );
  }, []);
  ...
  return (
    ...
    >
      <Grid container spacing={2.5}>
        <Grid item xs={12}>
          <SelectField name={"itemId"} label={"ItemID"} options={items.map(item => new Option(item.name, item.id))}/>
        </Grid>
        <Grid item xs={12}>
